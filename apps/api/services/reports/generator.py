"""
PDF report generator for SONGGATE scan results.
...
def _on_body_page(canvas, doc):
    ...
    canvas.setFillColor(C_ACCENT)
    canvas.drawString(MARGIN_L, PAGE_H - MARGIN_T + 4*mm, "SONGGATE")
...
SEVERITY_ORDER = {"critical": 0, "error": 0, "warning": 1, "info": 2}
LAYER_ORDER = ["ddex", "metadata", "fraud", "artwork"]
...
class ReportGenerator:
    ...
    def build(self, data: ReportData) -> bytes:
        ...
        doc = BaseDocTemplate(
            ...
            title=f"SONGGATE Report — {data.release_title}",
            author="SONGGATE",
        )
    ...
    def _cover_page(self, data: ReportData, styles: dict) -> list:
        ...
        # Wordmark
        story.append(Paragraph("SONGGATE", styles["cover_wordmark"]))
        story.append(Paragraph("Release QA Platform · songgate.io", styles["cover_tagline"]))
        ...
        story.append(Paragraph(
            ...
            "Generated automatically by SONGGATE — songgate.io",
    ...
    def _enrichment_page(self, suggestions: list[ReportSuggestion], styles: dict) -> list:
        ...
        story.append(HRFlowable(width=CONTENT_W, thickness=0.5, color=C_BORDER,
                                spaceBefore=0, spaceAfter=4*mm))
        story.append(Paragraph(
            "End of Report  ·  SONGGATE · songgate.io",
